#извлекаем из бдшки инофрмацию по человеку и переводим это в строку
#(на вопрос как это будет выглядеть я не знаю)


#я решил возвращать список задач. каждая задача - кортеж
from task_db.config_bd import DB_CONFIG
import asyncpg

async def get_task_from_db(tg_user_id, filters: dict, limit = 5):
    
    conn = None
    try:
        conn = await asyncpg.connect(**DB_CONFIG)

        query = """
            SELECT id, task_number, condition, difficulty 
                FROM tasks t
                WHERE NOT EXISTS (
                    SELECT 1 FROM user_progress up 
                    WHERE up.task_id = t.id AND up.user_id = $1
            )
        """

        params = [tg_user_id]

        if filters.get("task_number"):
            query += f" AND t.task_number = ${len(params) + 1}"
            params.append(int(filters["task_number"]))
        
        if filters.get("difficulty"):
            query += f" AND t.difficulty = ${len(params) + 1}"
            params.append(filters["difficulty"])
        
        query += f" ORDER BY RANDOM() LIMIT ${len(params) + 1}"
        params.append(limit)

        rows = await conn.fetch(query, *params)
        return rows

    except Exception as e:
        print(f"SQL Error: {e}")
        return []
    finally:
        if conn:
            await conn.close()